...
std::vector<direction> Player::checkDanger(const IndieVector3 &playerPosition) {
    ...
    for (IndieBomb &bomb: indie.bombs) {
        ...
        if (!right && bombPosition.getX() == round(playerPosition.getX()) &&
            bombPosition.getZ() > round(playerPosition.getZ()) &&
            round(playerPosition.getZ()) < bombPosition.getZ() + bomb.getForce()) {
            danger.push_back(RIGHT);
            right = true;
        } else if (!left && bombPosition.getX() == round(playerPosition.getX()) &&
                   bombPosition.getZ() < round(playerPosition.getZ()) &&
                   round(playerPosition.getZ()) > bombPosition.getZ() - bomb.getForce()) {
            danger.push_back(LEFT);
            left = true;
        } else if (!up && bombPosition.getZ() == round(playerPosition.getZ()) &&
                   bombPosition.getX() > round(playerPosition.getX()) &&
                   round(playerPosition.getX()) < bombPosition.getX() + bomb.getForce()) {
            danger.push_back(UP);
            up = true;
        } else if (!down && bombPosition.getZ() == round(playerPosition.getZ()) &&
                   bombPosition.getX() < round(playerPosition.getX()) &&
                   round(playerPosition.getX()) > bombPosition.getX() - bomb.getForce()) {
