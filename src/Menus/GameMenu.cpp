...
void Indie::displayGameMenu()
{
    ...
    static IndieButton backToGameButton({middle_x + 575, middle_y + 425, static_cast<float>(backToGame.getWidth()), static_cast<float>(backToGame.getHeight())},
    ...
    static IndieButton saveGameButton({middle_x + 575, middle_y + 525, static_cast<float>(saveGame.getWidth()), static_cast<float>(saveGame.getHeight())},
                                      {middle_x + 575, middle_y + 525}, saveGame, loader.textures["save_game_highlight"], [&]() -> void
                                      { buttonSound.Play(); });

    static IndieButton quitGameButton({middle_x + 999, middle_y + 525, static_cast<float>(quitGame.getWidth()), static_cast<float>(quitGame.getHeight())},
                                      {middle_x + 999, middle_y + 525}, quitGame, loader.textures["quit_to_title_highlight"], [&]() -> void
                                      {
        buttonSound.Play();
        this->state = mainMenu;});

    static IndieButton saveAndQuitButton({middle_x + 575, middle_y + 625, static_cast<float>(saveAndQuit.getWidth()), static_cast<float>(saveAndQuit.getHeight())},
                                         {middle_x + 575, middle_y + 625}, saveAndQuit, loader.textures["save_and_quit_highlight"], [&]() -> void
                                         {
        buttonSound.Play();
        this->state = mainMenu; });
