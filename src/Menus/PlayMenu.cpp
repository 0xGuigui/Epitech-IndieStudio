...
void Indie::displayPlayMenu() {
    ...
    static IndieButton playButtonObject({middle_x + 200, middle_y + 850, static_cast<float>(playButton.getWidth()),
                                         static_cast<float>(playButton.getHeight())},
                                        {middle_x + 200, middle_y + 850}, playButton, loader.textures["play_highlight"],
                                        [&]() -> void {
                                            buttonSound.Play();
    ...
    static IndieButton cancelButtonObject({middle_x + 1100, middle_y + 850, static_cast<float>(cancelButton.getWidth()),
                                           static_cast<float>(cancelButton.getHeight())},
                                          {middle_x + 1100, middle_y + 850}, cancelButton,
                                          loader.textures["cancel_highlight"], [&]() -> void {
                buttonSound.Play();
                this->state = mainMenu;
            });
